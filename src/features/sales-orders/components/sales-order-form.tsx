...
import { createEmptySalesOrderItem, getItemPrefill } from '../api/sales-orders.api'
import { salesOrderSchema, type SalesOrderSchema } from '../schemas/sales-order.schema'
import { useSalesOrderDefaults } from '../hooks/use-sales-order-defaults'
...
export function SalesOrderForm({
  ...
  const defaultsQuery = useSalesOrderDefaults()
  const {
    control,
    getValues,
    handleSubmit,
    ...
    defaultValues: {
      ...
      items: initialValues?.items?.length ? initialValues.items : [createEmptySalesOrderItem(initialValues?.delivery_date)],
    },
  })
  ...
  const orderDeliveryDate = useWatch({
    control,
    name: 'delivery_date',
  }) ?? ''
  ...
  useEffect(() => {
    ...
    if (!getValues('selling_price_list') && defaultsQuery.data.priceLists[0]?.name) {
      setValue('selling_price_list', defaultsQuery.data.priceLists[0].name)
    }
  ...
  function addItemRow() {
    append(createEmptySalesOrderItem(orderDeliveryDate))
  }
    ...
    <form className="form-layout" onSubmit={handleSubmit(async (values) => onSubmit(values))}>
      <div className="form-header">
        <div>
          ...
          <h3>{initialValues ? 'تعديل أمر بيع' : 'إنشاء أمر بيع'}</h3>
          <p>هذا المستند يربط العميل والأصناف والأسعار مع منطق ERPNext الفعلي في خطوة واحدة.</p>
        </div>
        <button className="button button-primary" disabled={disabled} type="submit">
          <Save size={17} aria-hidden="true" />
          {isSubmitting ? 'جاري الحفظ' : 'حفظ أمر البيع'}
        </button>
      </div>
      ...
      <div className="form-content">
        <div className="form-main">
          <section className="form-section">
            <div className="section-heading">
              <h4>رأس المستند</h4>
              <p>حقول أساسية تتحكم في الشركة والعميل والتاريخ والعملات داخل أمر البيع.</p>
            </div>
            <div className="field-grid">
              <label className="field" htmlFor="customer">
                <span>العميل</span>
                <LinkDatalistInput
                  id="customer"
                  disabled={disabled}
              ...
              <label className="field" htmlFor="company">
                ...
                <LinkDatalistInput
                  ...
                  listId="sales-order-company-options"
              ...
              <label className="field" htmlFor="delivery_date">
                <span>تاريخ التسليم</span>
                <input id="delivery_date" disabled={disabled} type="date" {...register('delivery_date')} />
              </label>
              ...
              <label className="field" htmlFor="currency">
                ...
                <LinkDatalistInput
                  ...
                  listId="sales-order-currency-options"
                  registration={register('currency')}
                />
              </label>

              <label className="field" htmlFor="selling_price_list">
                ...
                <LinkDatalistInput
                  ...
                  listId="sales-order-price-list-options"
              ...
              <label className="field" htmlFor="set_warehouse">
                ...
                <LinkDatalistInput
                  ...
                  disabled={disabled}
                  doctype="Warehouse"
                  listId="sales-order-warehouse-options"
          ...
          <section className="form-section">
            <div className="section-heading split">
              <div>
                <h4>أصناف الطلب</h4>
                <p>كل صف يمثل عنصرًا حقيقيًا في جدول `Sales Order Item` داخل ERPNext.</p>
              </div>
                    ...
                    <div className="line-item-grid">
                      <label className="field" htmlFor={`items.${index}.item_code`}>
                        ...
                        <LinkDatalistInput
                          ...
                          listId={`sales-order-item-options-${index}`}
                      ...
                      <label className="field" htmlFor={`items.${index}.uom`}>
                        ...
                        <LinkDatalistInput
                          ...
                          listId={`sales-order-uom-options-${index}`}
                      ...
                      <label className="field" htmlFor={`items.${index}.warehouse`}>
                        ...
                        <LinkDatalistInput
                          ...
                          disabled={disabled}
                          doctype="Warehouse"
                          listId={`sales-order-row-warehouse-options-${index}`}
                          registration={register(`items.${index}.warehouse`)}
                        />
                      </label>

                      <label className="field" htmlFor={`items.${index}.delivery_date`}>
                        <span>تاريخ التسليم</span>
                        <input id={`items.${index}.delivery_date`} disabled={disabled} type="date" {...register(`items.${index}.delivery_date`)} />
                      </label>
        ...
        <aside className="form-aside" aria-label="ملخص أمر البيع">
