...
import type { PurchaseOrderItem } from '../types/purchase-order.types'

export function PurchaseOrderItemsTable({ rows }: { rows: PurchaseOrderItem[] }) {
  const columns: DataColumn<PurchaseOrderItem>[] = [
    {
      key: 'item_code',
      header: 'كود الصنف',
  ...
  return <DataTable columns={columns} data={rows} getRowKey={(item) => item.name ?? `${item.item_code}-${item.schedule_date ?? 'row'}`} />
}
