import { CircleDollarSign, Clock3, FileText, Plus, Search, ShoppingBasket } from 'lucide-react'
...
import { usePurchaseOrderSummary, usePurchaseOrders } from '../hooks/use-purchase-orders'
...
export default function PurchaseOrdersListPage() {
  ...
  const summaryQuery = usePurchaseOrderSummary({ search, lifecycle, company })
  ...
  return (
    <>
      <Breadcrumbs items={[{ label: 'أوامر الشراء' }]} />
      <PageHeader
        actions={
        ...
        subtitle="إدارة أوامر الشراء المرتبطة بالموردين والأصناف والإجماليات داخل ERPNext."
        title="أوامر الشراء"
      />

      <section className="metrics-grid" aria-label="ملخص أوامر الشراء">
        <MetricCard
      ...
      <section className="toolbar" aria-label="فلاتر أوامر الشراء">
      ...
      {!isLoading && !isError && rows.length === 0 ? (
        <EmptyState
          ...
          message="جرّب تعديل الفلاتر أو أنشئ أول أمر شراء."
          title="لا توجد أوامر شراء."
        />
      ) : null}
      {!isLoading && !isError && rows.length > 0 ? <PurchaseOrdersTable permissions={permissions} rows={rows} /> : null}
